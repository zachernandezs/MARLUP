...
- `EXPERIMENTS/` — per-person sandbox copies for independent tuning experiments: `Aaron/`, `Alvaro/`, `Will/`, each with its own `script_<name>.m` + `Model_<name>.slx` pair (currently identical copies of `MODELS/marlup_init.m` / `marlup_model.slx`, meant to diverge as each person tunes gains). **Known bug**: each copy's `addpath(...'..','CAD')` is relative to the script's own folder, which is now two levels deep (`EXPERIMENTS/<name>/`) — it resolves to the nonexistent `EXPERIMENTS/CAD` instead of the real top-level `CAD/`, so it silently fails to put the CAD parts on the path unless a prior run of `MODELS/marlup_init.m` already added them in that MATLAB session.
